...
namespace SilkShield_New.Data
{
    public class InvoiceDataService
    {
        ...
        // FIXED: retrieve items by InvoiceNumber -> get InvoiceId then read InvoiceItems by InvoiceId
        public List<InvoiceItem> GetInvoiceItemsByNumber(string invoiceNumber)
        {
            ...
            using (var connection = _dbHelper.GetConnection())
            {
                ...
                // 1) find InvoiceId for the requested invoice number
                string idQuery = "SELECT InvoiceId FROM Invoices WHERE InvoiceNumber = @InvoiceNumber LIMIT 1";
                object idObj;
                using (var idCmd = new SQLiteCommand(idQuery, connection))
                {
                    idCmd.Parameters.AddWithValue("@InvoiceNumber", invoiceNumber);
                    idObj = idCmd.ExecuteScalar();
                }

                if (idObj == null || idObj == DBNull.Value)
                {
                    // invoice not found -> return empty list
                    return items;
                }

                long invoiceId = Convert.ToInt64(idObj);
